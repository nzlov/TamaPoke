...
g++ -std=c++17 -O1 -w \
  ...
  -o tamapoke-emu \
  sketch.cpp wavout.cpp "$ROOT/gbsynth.cpp" "$ROOT/art_codec.cpp" "$ROOT/content.cpp" "$ROOT/font_engine.cpp" "$ROOT/motion.cpp" "$ROOT/nature.cpp" "$ROOT/perf.cpp" "$ROOT/player.cpp" "$ROOT/pet.cpp" "$ROOT/quiz.cpp" "$ROOT/i18n.cpp" "$ROOT/party.cpp" "$ROOT/breeding.cpp" "$ROOT/inventory.cpp" "$ROOT/items.cpp" "$ROOT/wild.cpp" "$ROOT/battle.cpp" "$ROOT/link.cpp" "$ROOT/save.cpp" \
