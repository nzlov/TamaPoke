...
for src in "$HERE"/*_test.cpp; do
  ...
  elif [[ "$name" =~ ^(bag_ui|battle_reward_ui|battle_turn_animation|bond_battle|gender|box|nature|revive|save|link|hit|wild_traits|lifecycle_exit|flush|i18n|minigame_touch|cultivation_team|sleep|item_effect|poweroff|gym_atomic|roster_snapshot|lead|upgrade)_test$ ]]; then
