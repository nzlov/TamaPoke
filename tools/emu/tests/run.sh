...
needs_sketch() { case "$1" in touch_test|flush_test|joy_test|anim_test|battle_turn_animation_test|capture_animation_test|swipe_test|lan_test|console_test|hit_test|battle_debounce_test|battle_reward_ui_test|bag_ui_test|move_stone_ui_test|boot_order_test|brightness_test|card_nature_ui_test|first_boot_language_test|starter_test|recovery_test|navigation_test|wild_detail_test|missing_pack_roster_test|poweroff_test|lead_test) return 0;; *) return 1;; esac; }
...
for src in "$HERE"/*_test.cpp; do
  ...
  elif [[ "$name" =~ ^(bag_ui|battle_reward_ui|battle_turn_animation|bond_battle|gender|box|nature|revive|save|link|hit|wild_traits|lifecycle_exit|flush|i18n|cultivation_team|sleep|item_effect|poweroff|gym_atomic|roster_snapshot|lead|upgrade)_test$ ]]; then
